#' Enrichment Dot Plot
#'
#' Visualize the enrichment results of transcriptional programs using a dot plot. The function takes a list of enrichment results generated by \code{\link{PGEnricher}} as input and plots the top enriched terms for each program by \code{\link{ggplot2}}.
#'
...
#'
#' @param only.top A logical value indicating whether to show only the top enriched terms (TRUE) for each program or show all enriched terms that belong to the union of top enriched pathways (FALSE).
#'
...
#' @export
#'
EnrichDotPlot <- function(enrich.list,
                          top = 3,
                          only.top = TRUE,
                          title = "Enriched GeneSets",
                          color = paletteer_dynamic("cartography::red.pal", 20)[10:20]) {
  if (inherits(enrich.list[[1]], "enrichResult")) {
    enrich.list <- lapply(enrich.list, function(enrich) {
      enrich@result
    })
  }
